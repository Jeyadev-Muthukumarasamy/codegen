...
const ShowCase = () => {
  ...
  const getShowCaseUsers = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:3001/api/getShowCase?email=${email}`);
  ...
  return (
    <div className='min-h-screen bg-gray-900 flex flex-row'>
      ...
      <div className='p-8 flex-1'>
        <h1 className='text-white text-4xl font-bold mb-4 border-b border-blue-500 pb-4'>
          Showcased Projects
        </h1>
        <p className='text-white text-lg mb-8 font-sans font-semibold'>
          These are projects showcased by people. Browse and explore their amazing work!
        </p>
