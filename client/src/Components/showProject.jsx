...
const CodeChat = () => {
  ...
  const loadProject = async () => {
    try {
      ...
      if (data) {
        const formattedCode = beautify.html(data.formattedCode || '', { 
          indent_size: 2, 
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const lastBotMessage = messages.findLast(msg => msg.sender === 'bot')?.text || '';
      
      const response = await axios.post('http://localhost:3001/update', {
        inputMessage: input,
        existingCode: finalCodeValue,
        aiMessage: lastBotMessage
      });
      ...
      if (response.data.frontendMessage) {
        ...
        await axios.post(`http://localhost:3001/api/updateCodeById?id=${id}`, {
          formattedCode: response.data.code || finalCodeValue,
  ...
  const handleActions = {
    ...
    save: async () => {
      ...
      try {
        ...
        setMessages(prev => [...prev, { 
          text: 'Error saving code. Please try again.', 
          sender: 'bot' 
        }]);
      } finally {
        setLoading(false);
  ...
  return (
    <div className="flex flex-col h-screen bg-[#1E1E1E]">
      ...
      {/* Mobile Home Button */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 p-4 bg-[#252526] border-t border-[#3C3C3C]">
        <button
          onClick={handleActions.goHome}
